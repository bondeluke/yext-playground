## Thrivent Specific Information

### Serving Locator Pages Locally

This site has multiple static locator pages, on prod located on the root page (Search by location locator) and at "/search-by-name" (Search by name locator). The url paths for these locators are hardcoded and not using slug fields because they're using the same template powered by a different Search vertical.

In order to serve the locator pages locally, you MUST use `npx pages dev --no-prod-url`

### Specialty Filter Logic

The Serivces filters on the locators is quite customized. Each filter choice represents a set of different values coming from the `c_designations` field, as outlined below:

- Financial Planning: c_designations includes one or more of "AAMS®", "BFA™", "CFP®", "ChFC®", "MSPFP"
- Financial Education: c_designations includes "RICP®"
- Estate/Wealth Management: c_designations includes one or more of "WMCP®", "CPWA®"
- College Planning: c_designations includes "RICP®"
- Divorce Planning: c_designations includes "RICP®"
- Legacy Planning: c_designations includes "RICP®"
- Investment Management: c_designations includes one or more of"CPM", "CIPM", "CIMA®"
- Insurance: c_designations includes one or more of "RHU®", "LUTCF®", "FSA", "FPC", "FLMI", "FICF", "FIC", "CLU®", "CLTC®"
- Retirement Planning: c_designations includes one or more of "RICP®", "CRPS®", "CPRC"
- General Planning: c_designations includes "CFA®"

To edit this functionality, adjust the `filterMappings` variable in `src/components/search/SpecialtyFilter.tsx`
