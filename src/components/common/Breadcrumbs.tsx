...
interface BreadcrumbsPropsDefault {
  breadcrumbs: Array<{ slug: string; name: string; c_breadcrumb: string }>;
  separator?: ReactNode;
  className?: string;
}
 ...
 * ```
 *
 * @param {[{name: string, slug: string, breadcrumb: string}]} breadcrumbs
      ...
      {breadcrumbs?.length && (
        ...
        >
          <ol className="flex flex-wrap">
            {breadcrumbs.map(({ name, slug, c_breadcrumb }, idx) => {
              const isLast = idx === breadcrumbs.length - 1;
              const isFirst = idx === 0;

              return (
                <li key={idx} className={isLast ? "font-bold" : ""}>
                  <Breadcrumb
                    name={c_breadcrumb}
